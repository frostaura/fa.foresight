...
public static class DatabaseInitializer
{
    public static async Task InitializeAsync(IServiceProvider services, CancellationToken ct = default)
    {
        ...
        var secretProtector = scope.ServiceProvider.GetRequiredService<Security.ISecretProtector>();
        ...
        await db.Database.ExecuteSqlRawAsync(@"
            ...
            -- Per-tenant platform connection: wallet key (encrypted), signing params, endpoints,
            -- and the LiveTrading gate now live in the DB. The IsDefault row is the active connector.
            CREATE TABLE IF NOT EXISTS platform_connections (
                ""Id""                  uuid PRIMARY KEY,
                ""TenantId""            uuid NOT NULL,
                ""ConnectorId""         varchar(60) NOT NULL,
                ""IsDefault""           boolean NOT NULL DEFAULT false,
                ""PrivateKeyEncrypted"" text NULL,
                ""ApiSecretEncrypted""  text NULL,
                ""WalletAddress""       varchar(64) NULL,
                ""SignatureType""       integer NOT NULL DEFAULT 0,
                ""Funder""              varchar(64) NULL,
                ""ClobBaseUrl""         varchar(300) NOT NULL,
                ""GammaBaseUrl""        varchar(300) NOT NULL,
        ...
        // DB row is authoritative and editable in-app — env is never re-read for the connection.
        await SeedDefaultPlatformConnectionAsync(db, config, secretProtector, logger, ct);
    ...
    /// <summary>
    /// One-time bootstrap of the default tenant's platform connection from env vars. If the default
    /// tenant already has a platform_connections row, this is a no-op (the DB row is authoritative).
    /// The wallet private key is ENCRYPTED via <see cref="Security.ISecretProtector"/> before storage;
    /// the wallet address is derived from the key (Nethereum) when present.
    /// </summary>
    private static async Task SeedDefaultPlatformConnectionAsync(
        ForesightDbContext db,
        IConfiguration config,
        Security.ISecretProtector protector,
        ILogger logger,
        CancellationToken ct)
    {
        var defaultTenant = await db.Tenants.FirstOrDefaultAsync(t => t.Slug == "default", ct);
        ...
        if (hasKey)
        {
            try
            {
                var ethKey = new EthECKey(key.StartsWith("0x", StringComparison.OrdinalIgnoreCase) ? key : "0x" + key);
                walletAddress = ethKey.GetPublicAddress();
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, "Bootstrap: could not derive wallet address from KeyVault:PrivateKey — seeding connection without an address");
            }
        }
        ...
        logger.LogInformation(
            "Bootstrapped default-tenant platform connection (connector=polymarket, hasKey={HasKey}, liveTrading={LiveTrading}) from env",
            hasKey, liveTrading);
    }
