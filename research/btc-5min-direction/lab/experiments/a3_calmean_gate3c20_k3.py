"""a3 ensemble agreement gating — calibrated-mean + all-3 concordance gate.

Wrapper fits LGBM (baseline config) + XGBoost (hist, similar size) + logistic
(standardized, C=0.5) on the first 90% of the fit rows, per-model isotonic
calibration on the last 10% (purged 63 bars). predict_proba returns the
equal-weight mean of the three CALIBRATED probabilities, shrunk fully to 0.5
when the three calibrated sides disagree (Sebastiao & Godinho: only concordant
calls survive costs). A tiny raw-mean tie-breaker keeps ordering strict inside
isotonic plateaus without ever flipping a side. The harness then applies its
own isotonic on top and selects by |p-0.5| from calib quantiles as usual.
"""
import numpy as np
...
NAME = "a3_calmean_gate3c20_k3"
K = 3
PRIMARY_COV = 0.025
EMB_INT = 63  # K + 60, internal purge
