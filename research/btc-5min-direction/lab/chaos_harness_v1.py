"""CHAOS HARNESS v1 — the frozen evaluator for the 2026-06 fa.foresight campaign.
...
def main():
    ...
    for w in range(12):
        ...
        s_cal = np.abs(p_cal - 0.5) if score_fn is None else np.asarray(score_fn(p_cal, df.iloc[cal_idx]))
        s_te = np.abs(p_te - 0.5) if score_fn is None else np.asarray(score_fn(p_te, df.iloc[te]))
    ...
    out = dict(name=name, K=K, primary_cov=prim, dataset_sha256=dhash, n_features=len(cols),
               ...
               bust_test_primary=bust, harness="chaos_harness_v1", ts=int(time.time()))
    os.makedirs(RESULTS, exist_ok=True)
    with open(os.path.join(RESULTS, f"{name}.json"), "w") as f: json.dump(out, f, indent=1)
    with open(LEDGER, "a") as f:
        f.write(json.dumps(dict(name=name, K=K, primary_cov=prim, ts=out["ts"], dataset=dhash[:16],
